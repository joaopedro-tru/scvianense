...
export default function App() {
  ...
  const handleSubmit = () => {
    ...
    if (!num || !jogo) return;
    if (socios[num]) {
      setResultado({ tipo: "dup", socio: num, ...socios[num] });
    } else {
      const agora = new Date().toISOString();
      const novosSocios = { ...socios, [num]: { local, timestamp: agora } };
      const novoHist = [{ socio: num, local, timestamp: agora }, ...historico];
      setSocios(novosSocios);
      setHistorico(novoHist);
      save(novosSocios, novoHist);
